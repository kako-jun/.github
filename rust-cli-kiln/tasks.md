...
- [ ] **diffx統一API設計思想違反修正**: 単一diff()関数のみ公開、パーサー関数群削除して統一思想準拠
- [ ] **全プロジェクトcore APIにディレクトリ再帰処理（--recursive）実装**: CLIオプションの完全API対応

### 修正された認識

- ~~diffai言語バインディング不完全問題修正~~: **誤認** - diffaiは正しく統一API思想に準拠済み
- ~~lawkit言語バインディング不完全問題修正~~: **誤認** - lawkitは正しく統一API思想に準拠済み

**正しい状況**: diffai/lawkitは単一関数のみ公開で統一API設計思想に準拠。diffxのみが思想違反（余分な関数公開）